...
//     // checking if there are no log entries for the current date
//     if (!logData || !logData.some((entry) => entry.date === date)) {
//       try {
//         const requestBody = {
//           user: userId,
...
export default function LogTable({ checkInTimeData, selectedDate }) {
  const [logEntries, setLogEntries] = useState([]);
  const { data: logData, isLoading: logIsLoading } =
    useDetailAllTrackingLogQuery({
      date: selectedDate, // Passing the selected date as a filter
    });
  ...
  return (
    <div>
      <h2> Log Entries for {selectedDate}</h2>
      {logIsLoading ? (
        ...
        <TableContainer component={Paper} className="custom-table-container">
          <Table sx={{ minWidth: 700 }} aria-label="customized table">
            ...
            <TableBody>
              ...
              {logEntries.map((index) => (
                <StyledTableRow key={index}>
                  <StyledTableCell align="center">1</StyledTableCell>
                  <StyledTableCell aligh="center">
                    {formattedDate}
                  </StyledTableCell>
                  <StyledTableCell align="center">
                    {checkInTimeData?.data?.checkInTime?.title}
